...
solver with a 100-evaluation budget and `1e-7` step tolerance in Excel-compatible mode;
`ExtendedSearch` widens the budget and tightens the tolerance. `YIELD(N=1)` and `ODDLYIELD` are
direct closed forms and never run the solver.

Two boundary behaviors follow the frozen worksheet observations rather than the general Microsoft
documentation alone: `TBILLEQ` and `TBILLPRICE` accept `settlement == maturity`, while
`TBILLYIELD` requires `settlement < maturity`; and a T-bill discount that would produce a
non-positive price returns `#NUM!`. `TBILLEQ` uses the documented simple form through 182 actual
days and the bond-equivalent square-root form above 182 days.

Independent literal fixtures pin the one-coupon `PRICE`/`YIELD` branches, `PRICEMAT`/`YIELDMAT`,
odd-last price/yield fractions, and the odd-first short price, so no expected value is generated by
reusing the production kernel on both sides.
